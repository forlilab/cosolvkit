...
class Report:
    ...
    def __init__(self, statistics_file, traj_file, top_file, cosolvent_names, out_path):
        ...
        :param statistics_file: log file generated by MD Simulation. In CosolvKit this is called statistics.csv.
            Can be a list of files to concatenate multiple runs.
        :type statistics_file: Union[str, list]
        :param traj_file: Trajectory file generated by MD Simulation.
            Can be a list of files to concatenate multiple runs.
        :type traj_file: Union[str, list]
        ...
        if isinstance(cosolvent_names, str):
        ...
        self.trajectory = [traj_file] if isinstance(traj_file, str) else traj_file
    ...
    def _get_temp_vol_pot(self, statistics_file):
        ...
        """
        if isinstance(statistics_file, list):
            df = pd.concat([pd.read_csv(f) for f in statistics_file], ignore_index=True)
        else:
    ...
    def generate_pymol_session(self, 
        ...
        if density_files is None:
            density_files = [os.path.join(self.out_path, f) for f in os.listdir(self.out_path) if f.endswith('.dx')]
            if len(density_files) == 0:
                self.logger.error("No .dx density files found in the output directory. Run generate_density_maps() first.")
                return
            ...
            self.logger.error("Please provide a list of density files, a directory, or a single .dx file path.")
            return
        ...
        spectrum_selection = selection_string if selection_string else "all"
        cmd.spectrum("b", "blue_white_red", spectrum_selection)
        ...
        cmd_string += f"cmd.spectrum('b', 'blue_white_red', '{spectrum_selection}')\n"
    ...
    def generate_vmd_session(self, 
        ...
        traj_addfile_lines = "\n".join(
            f"    mol addfile {os.path.abspath(t)} type dcd waitfor all"
