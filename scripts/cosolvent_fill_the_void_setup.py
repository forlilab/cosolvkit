...
def build_cosolvent_box(receptor_path: str, 
                        cosolvents: str, 
                        forcefields: str, 
                        simulation_engine: str, 
                        output_path: str, 
                        radius: float, 
                        use_halton: bool) -> CosolventSystem:
    ...
    cosolv.build(use_halton=use_halton)
    return cosolv
...
def run_simulation(out_path, cosolv_system, simulation_time=None, simulation_engine="Amber", output_filename="simulation"):
    ...
    simulation.reporters.append(DCDReporter(os.path.join(results_path, output_filename + ".dcd"), 2500))
...
def cmd_lineparser():
    ...
    parser.add_argument('-halton', dest='halton', required=True,
                        action='store', help='Use halton sequence')
...
if __name__ == "__main__":
    ...
    use_halton = bool(args.halton)
    print("Building cosolvent box")
    cosolv_system = build_cosolvent_box(receptor_path, cosolvents, forcefields, simulation_engine, output_path, radius, use_halton)
